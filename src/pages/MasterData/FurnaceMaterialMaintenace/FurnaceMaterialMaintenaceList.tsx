...
const FurnaceMaterialMaintenaceList: FC = () => {
  //Constants
  ...
  const onConfirmClick = async () => {
    ...
    const deleteResponse = await dispatch(deleteFurnaceMaterial(requestData));
  ...
  const onToggleUpdate = async () => {
    ...
    const statusResponse = await dispatch(statusFurnaceMaterial(requestStatusData));
  ...
  const handleRowClick = ({ id, edit, isActive, view }: RowClickArguments) => {
    const url = `${
      edit ? paths.furnaceMaterialMaintenance.edit : paths.furnaceMaterialMaintenance.view
  ...
  return (
    <>
      {isLoading && <Loading />}
      <Header
        title='Furnace Material Maintenance'
...
export default FurnaceMaterialMaintenaceList;

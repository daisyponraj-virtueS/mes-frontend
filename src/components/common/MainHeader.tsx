...
const Header: React.FC<HeaderProps> = ({
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.length == 1 && value[0] === ' ') {
      ...
      emptyCheck ? onSearchChange?.(value)
  ...
  const handleFilterSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    let srLength: any = searchedResponse?.filter(
      (res: any) =>
        res.material_name
          .toString()
          .toLowerCase()
  ...
  const onSort_filterClick = ({ fromFilterSearch = true } = {}, searchText?: string) => {
    ...
    sort_filter_click?.(inputData, fromFilterSearch);
    setOpenFilter(false);
    setOpenSort(false);
                      ...
                      {!isEmpty(searchedResponse && searchedResponseDuplicate)
                        ? searchedResponse
                            ...
                            .map((item, index) => {
                              ...
                              return (
                                <div className='custom-checkbox custom-checkbox--md ' key={index} >
                                  <input
                                    type='checkbox'
                                    id={`${index}`}
                    ...
                    <div className='pt-3'>
                      <div className='filters-sort-dropdown-menu__list__container relative'>
                        ...
                        <ul className={`select-dropdown-menu ${openStatusList ? 'open' : ''}`}>
                          {statusTypeList.map((item: any, index: number) => {
                            return (
                              <div
                                key={index}
